...
import { getChannel, getChannels, GetDMs, getSelf } from '../../API/Chat';
...
export default function Chat() {
  ...
  useEffect(() => {
    ConsumeEffect(setIsCLoaded, setchannels, () => {
      return getChannels();
    });
  }, []);
  const [isLoadedConversation, setIsLoadedConversation] =
    useState<boolean>(false);
  const [Conversation, setConversation] = useState<Channel | undefined>(
    undefined
  ); //Person[]

  useEffect(() => {
    if (channelID != null) {
      ConsumeEffect(setIsLoadedConversation, setConversation, () => {
  ...
  useEffect(() => {
    ConsumeEffect(setIsLoadedDm, setDmChannel, () => {
      return GetDMs();
    });
  }, [reload]);
        ...
        <>
          ...
          <div className="NavBar">
            <Navdrawer
              ...
              imageLink={
                selfProfile != null
                  ? Pfp(selfProfile!.id!, selfProfile!.profilePicture!)
                  : ''
              }
            >
              {isLoadedDm &&
              ...
              DmChannel &&
              DmChannel.channels &&
              DmChannel.channels.length > 0 ? (
                <>
                  <Divider />
                  <div
                    onClick={() => {
                      setShowAllDms(!showAllDms);
                    }}
                    style={{ cursor: 'pointer' }}
                  >
                    <label className="label leftside navbar pointer">
                      Direct Messages
                    </label>
                    <div>
                      <ArrowForwardIcon
                        className={
                          showAllDms
                            ? 'arrowww arrow-forward-icon-active'
                            : 'arrowww arrow-forward-icon'
                        }
                        style={{
                          cursor: 'pointer',
                        }}
                  ...
                  {showAllDms
                    ? DmChannel.channels.map((dm) => (
                        <NavListItem
                          key={dm.id}
                          text={
                            dm.users?.find(
                              (user) => user.id !== selfProfile?.id
                            )?.username
                          }
                          icon={
                            <Avatar
              ...
              selfProfile ? (
                <div>
                  {channels.find(
                    (channel) => channel.type === 'NEWS_CHANNEL'
                  ) != null ? (
                    <>
                      <Divider />
                      <label className="label leftside navbar">
                        News Channels
                      </label>
                    </>
                  ) : (
                  ...
                  {channels.find(
                    (channel) => channel.type === 'PUBLIC_CHANNEL'
                  ) != null ? (
                    <>
                      <Divider />
                      <label className="label leftside navbar">
                  ...
                  {channels.map((channel) =>
                    ...
                    channel!.rolesAccess!.includes(selfProfile?.role) ? (
                      <NavListItem
                        key={channel.id}
                        text={channel.name}
            ...
            <ChatWindow
              imageLink={
                Conversation.type === 'DM'
                  ? Pfp(
                      DmChannel?.channels
                        ?.find((channel) => channel.id === Conversation.id)
                        ?.users?.filter(
                          (person) => person.id !== selfProfile?.id
                        )[0].id!,
                      DmChannel?.channels
                        ?.find((channel) => channel.id === Conversation.id)
                        ?.users?.filter(
                  ...
                  : 'https://cdn.rdmraceapp.nl/embed/avatars/default.png'
              }
              name={
                Conversation.type === 'DM'
                  ? DmChannel?.channels
