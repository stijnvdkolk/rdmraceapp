...
function FkDateTime(date: Date) {
  ...
  var retVal: string = `${dtm.getDate()}/${
    dtm.getMonth() + 1
  }/${dtm.getFullYear()} ${dtm.getHours()}:${dtm.getMinutes()}:${dtm.getSeconds()}`;
  return retVal;
}
interface UParams {
...
export default function ChatWindow(props: IProps) {
  ...
  const [LastClickedPerson, setLastCLickedPerson] = useState<
    string | undefined
  >(undefined);
  ...
  async function MakeDmAndGo() {
    if (LastClickedPerson) {
      ...
      await MakeDM(LastClickedPerson).then((res) => {
        handleClose();
  ...
  const [messageClicked, setmessageClicked] = useState<string | undefined>(
    undefined
  );
  ...
  function setlastclickedImage(e: EventTarget) {
    if ((e as HTMLImageElement).src === undefined) {
      if ((e as HTMLDivElement).firstChild !== null) {
        if (
          ((e as HTMLDivElement).firstChild as HTMLImageElement).src !==
          undefined
        ) {
          setLastCLicked(
            ((e as HTMLDivElement).firstChild as HTMLImageElement).src
  ...
  function setLastClickedPersonAttchment(e: EventTarget) {
    if (
      ...
      (e as HTMLImageElement).src &&
      (e as HTMLImageElement).src.length > 1
    ) {
      var temp: HTMLDivElement = (e as HTMLDivElement)
        .parentElement as HTMLDivElement;
      while (
        (temp.parentElement as HTMLDivElement).id !== '' &&
        (temp.parentElement as HTMLDivElement).id.length > 1
      ) {
  ...
  function setLastClickedPerson(e: EventTarget) {
    if (
      ...
      findLastPersonClicked(
        (e as HTMLImageElement).parentElement?.parentElement?.parentElement?.id!
      );
  ...
  useEffect(() => {
    ...
    ConsumeEffect(setIsMessageLoaded, setMessages, () => {
          ...
          <>
                    ...
                    <div className={'message'} id={berichten.id}>
                      ...
                      >
                        <Avatar
                          src={Pfp(
                            berichten.author?.id!,
                            berichten.author?.profilePicture!
                          )}
                          alt=""
                          sx={{
                            width: '50px',
                      ...
                      {berichten.attachments &&
                        berichten.attachments.map((attachment, index) => (
                          <div
                            key={index}
            ...
            <Menu
              ...
              anchorPosition={
                ProfilePictureContext !== null
                  ? {
                      top: ProfilePictureContext!.mouseY,
                      left: ProfilePictureContext!.mouseX,
                    }
                  : undefined
              }
            >
              ...
              <MenuItem
                onClick={() => downloadImage(CloseProfilePictureContext)}
              >
                <div className="MenuItemWithIcon">
              ...
              <MenuItem
                onClick={() => DeleteMessage(CloseProfilePictureContext)}
              >
                <div className="MenuItemWithIcon">
                  <DeleteOutlineOutlinedIcon color="primary" />
                  {'Delete Message'}
  ...
  const chatHeaderContainer = ( //we just put all children in a div over here so we don't have to worry about mismatches
    <div className="chatHeaderContainer">
      ...
      <div className="channelTrailing static"></div>
    </div>
  );
  //#endregion
  //#region messageInput
